...
def create_w(request, id):
    if (
        Register.objects.filter(id=id).exists()
        and Register.objects.get(id=id).sender == request.sender
    ):
    ...
    if not registers:
        ...
        for register in registers:
            if ids:  # Using optimized queries:
                objects = register.objects.filter(id__in=ids).values_list(
                    "id", flat=True
                )
            else:
            ...
            for chunk in chunks(objects, 20):
                ...
                t += 1
                tasks_map.append(
                    request_by_mapper(register, chunk, countdown, datetime.now())
                )
        g = group(*tasks_map)
    ...
    if request.method == "POST":
        ...
        if payment is None:
            ...
            post_params = {
                ...
                "currency1": "BTC",
                "currency2": currency,
                "buyer_email": request.user.email,  # TODO set request.user.mail,
            ...
            try:
                try:
                    ...
                    try:
                        ...
                        message = render_to_string(
                            "first_email.html",
                            {"user": policy.user, "payment": payment},
                        )
                    ...
                    logger.error(e)
                    responseData = {"error": True, "message": "Payment Gateway Error"}
                    return JsonResponse(responseData)
                else:
                    payment.save()
                ...
                post_params = {
                    "payment_amount": decimal.Decimal(transaction.amount)
                    .quantize(
                        decimal.Decimal("0.00000001"), rounding=decimal.ROUND_DOWN
                    )
                    .normalize(),
                    "payment_address": transaction.address,
                    "payment_qr": transaction.qrcode_url,
                    "gateway_status": transaction.status_url,
                    "policy_cover": policy.cover,
                    "exchange_name": policy.exchange.name,
                    "date_of_formating": policy.request_date.date(),
                    "currency": currency,
                }
            ...
            post_params = {
                "payment_amount": decimal.Decimal(transaction.amount)
                .quantize(decimal.Decimal("0.00000001"), rounding=decimal.ROUND_DOWN)
                .normalize(),
                "payment_address": transaction.address,
                "payment_qr": transaction.qrcode_url,
                "gateway_status": transaction.status_url,
...
def backup_to_csv(request):
    ...
    if request.method == "GET":
        ...
        header = [
            ...
            "Outstanding_claim_BTC",
            "Date_of_claim",
            "Paid_claim_BTC",
            "Date_of_claim_payment",
            "Insurance_policy_status",
            "User_payments_status",
...
def dashboard(request):
    ...
    try:
        userPartner = Partner.objects.get(django_user=request.user.id)
        logger.info("Partner: %s logged into system" % (userPartner))
        return account(request)
    except ObjectDoesNotExist:
        # handle case for regular user
        pass

    insurancy_policy_info = (
        Policy.objects.order_by("-id")
        .filter(user=user.id)
        .exclude(status=PolicyStatus.DELETED)
    )
    ...
    try:
        ...
        logger.error("Fee values hasn't been found for user with ID: " + str(user.id))
        found_fee_values = []
    ...
    try:
        ...
        logger.error(
            "Policy number hasn't been found for user with ID: " + str(user.id)
        )
    ...
    try:
        ...
        logger.error("Couldn't find start dates for user with ID: " + str(user.id))
        found_start_dates = []
    ...
    try:
        found_expiration_dates = insurancy_policy_info.values("expiration_date")
    except KeyError as error:
        logger.error(
            "Couldn't find expirations dates for user with ID: " + str(user.id)
        )
    ...
    try:
        ...
        logger.error(
            "Couldn't find limits of liability for user with ID: " + str(user.id)
        )
    ...
    try:
        found_dates_of_formatting = insurancy_policy_info.values("request_date")
    except KeyError as error:
        logger.error(
            "Couldn't find dates of formatting for user with ID: " + str(user.id)
        )
    ...
    try:
        ...
        logger.error("Couldn't find stock exchanges for user with ID: " + str(user.id))
    ...
    try:
        ...
        logger.error("Couldn't find policy statuses for user with ID: " + str(user.id))
    ...
    for current_id, policy_id in enumerate(insurancy_policy_info):
        ...
        try:
            context_policy_number = policy_number_tag + str(
                (policy_numbers[current_id])["id"]
            )
        ...
        try:
            ...
            logger.error(
                "An error has occured while trying to get fee. Reason: " + str(error)
            )
        ...
        try:
            ...
            context_insurance_period = "%s %s'%s - %s %s'%s" % (
                s_date.day,
                s_date.strftime("%B")[0:3],
                s_date.year - 2000,
                e_date.day,
                e_date.strftime("%B")[0:3],
                e_date.year - 2000,
            )
        ...
        try:
            ...
            logger.error(
                "An error has occured while trying to get limit . Reason: " + str(error)
            )
        ...
        # NOTE: filling "Crypto exchange" form
        try:
            exchange_tag = CryptoExchange.objects.filter(
                id=stock_exchange_ids[current_id]["exchange"]
            ).values("name")[0]
        ...
        try:
            policy_status_numerical_value = policy_statuses[current_id]["status"]
            policy_status_tag = get_policy_status_tag(policy_status_numerical_value)
        except (IndexError, KeyError) as error:
            logger.error(
        ...
        if start_dates[current_id]["start_date"]:
            days = expiration_dates[current_id][
                "expiration_date"
        ...
        context_policies = {
            ...
            "policy_number": context_policy_number,
            "insurance_period": context_insurance_period,
            "limit": context_limit,
            "stock": context_stock_exchange,
            "formatting_date": context_date_of_formatting,
            "amount_of_premium": decimal.Decimal(context_fee)
            .quantize(decimal.Decimal("0.00000001"), rounding=decimal.ROUND_DOWN)
            .normalize(),
            "status": policy_status_tag,
            "numstatus": policy_status_numerical_value,
            "sosexists": sos,
    ...
    for stock_exchange in stock_exchange_ids:
        current_stock_exchange = (
            CryptoExchange.objects.select_related()
            .filter(id=stock_exchange["exchange"])
            .values("name")[0]
        )["name"]
    ...
    for stock_exchange in stock_exchange_tags:
        coverage_limit = (
            CryptoExchange.objects.select_related()
            .filter(name=stock_exchange)
            .values("coverage_limit")[0]
        )["coverage_limit"]
        ...
        for policy in contextPolicy:
            if policy["stock"] == current_stock_exchange and (
                policy["numstatus"] == 1 or policy["numstatus"] == 2
            ):  # BEFORE 2
